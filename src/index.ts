...
export const Config: Schema<Config> = Schema.object({
    ...
    platform_name: Schema.string().default('default').description('消息平台名称'),
    data_sources: Schema.object({
        fan_studio: Schema.object({
            ...
            japan_jma_eew: Schema.boolean().default(true).description('日本气象厅（JMA）：紧急地震速报'),
            usgs_earthquake: Schema.boolean().default(true).description('美国地质调查局（USGS）：地震测定'),
            china_weather_alarm: Schema.boolean().default(true).description('中国气象局：气象预警'),
            china_tsunami: Schema.boolean().default(true).description('自然资源部海啸预警中心：海啸预警信息'),
        }).description('FAN Studio WebSocket 数据源'),
        p2p_earthquake: Schema.object({
            enabled: Schema.boolean().default(true).description('启用P2P地震情報数据源'),
        ...
        wolfx: Schema.object({
            enabled: Schema.boolean().default(true).description('启用Wolfx数据源'),
            japan_jma_eew: Schema.boolean().default(true).description('日本気象庁：緊急地震速報'),
        ...
        global_quake: Schema.object({
            enabled: Schema.boolean().default(true).description('启用Global Quake数据源'),
